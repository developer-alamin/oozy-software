  ...
  <v-card outlined class="mx-auto my-5" max-width="">
    ...
    <v-card-text>
      <v-form ref="form" v-model="valid" @submit.prevent="submit">
        <v-text-field
          v-model="breakdown_service.name"
        ...
        <v-row>
          <v-col cols="6">
            <v-autocomplete
              v-model="breakdown_service.company_id"
              :items="companys"
              item-value="id"
              item-title="name"
              outlined
          ...
          <v-col cols="6">
            <v-autocomplete
              v-model="breakdown_service.factory_id"
              :items="factories"
              item-value="id"
              item-title="name"
        ...
        <v-row>
          <v-col cols="6">
            <v-autocomplete
              v-model="breakdown_service.brand_id"
              :items="brands"
              item-value="id"
              item-title="name"
          ...
          <v-col cols="6">
            <v-autocomplete
              v-model="breakdown_service.model_id"
              :items="models"
              item-value="id"
              item-title="name"
        ...
        <v-row>
          <v-col cols="6">
            <v-autocomplete
              v-model="breakdown_service.mechine_type_id"
              :items="types"
              item-value="id"
              item-title="name"
          ...
          <v-col cols="6">
            <v-text-field
              v-model="breakdown_service.preventive_service_days"
              :rules="[rules.required]"
        ...
        <v-row>
          <v-col cols="6">
            <v-text-field
              v-model="breakdown_service.mechine_code"
              label="Mechine Code"
              outlined
              density="comfortable"
              :error-messages="errors.mechine_code ? errors.mechine_code : ''"
            >
            </v-text-field>
          </v-col>
          <v-col cols="6">
            <v-autocomplete
              v-model="breakdown_service.mechine_source_id"
              :items="sources"
              item-value="id"
              item-title="name"
        ...
        <v-row>
          <v-col cols="6">
            <v-autocomplete
              v-model="breakdown_service.rent_id"
              :items="rents"
              item-value="id"
              item-title="name"
              label="Select Rent"
          ...
          <v-col cols="6">
            <v-date-input
              v-model="breakdown_service.rent_date"
              label="Rent Date"
              density="comfortable"
        ...
        <v-row>
          <v-col cols="4">
            <v-autocomplete
              v-model="breakdown_service.supplier_id"
              :items="suppliers"
              item-value="id"
              item-title="name"
          ...
          <v-col cols="4">
            <v-date-input
              v-model="breakdown_service.purchase_date"
              label="Purchase Date"
              density="comfortable"
          ...
          <v-col cols="4">
            <v-text-field
              v-model="breakdown_service.purchace_price"
  ...
  methods: {
    async fetchMechine() {
      ...
      try {
        ...
        this.breakdown_service = response.data.mechineAssing;
        this.breakdown_service.status = this.statusItems.includes(
          this.breakdown_service.status
        )
          ? this.breakdown_service.status
          : "";
        ...
        const selectedCompany = this.companys.find(
          (c) => c.id === this.breakdown_service.company_id
        );
        if (selectedCompany) {
          this.breakdown_service.company_id = selectedCompany.id; // Set the company_id for v-autocomplete
        }
        // factories
        const selectedFactory = this.factories.find(
          (c) => c.id === this.breakdown_service.factory_id
        );
        if (selectedFactory) {
          this.breakdown_service.factory_id = selectedFactory.id; // Set the company_id for v-autocomplete
        }
        // Brands
        const selectedBrand = this.factories.find(
          (c) => c.id === this.breakdown_service.brand_id
        );
        if (selectedBrand) {
          this.breakdown_service.brand_id = selectedBrand.id; // Set the company_id for v-autocomplete
        }

        // Models
        const selectedModel = this.factories.find(
          (c) => c.id === this.breakdown_service.model_id
        );
        if (selectedModel) {
          this.breakdown_service.model_id = selectedModel.id; // Set the company_id for v-autocomplete
        }

        // Type
        const selectedType = this.factories.find(
          (c) => c.id === this.breakdown_service.mechine_type_id
        );
        if (selectedType) {
          this.breakdown_service.mechine_type_id = selectedType.id; // Set the company_id for v-autocomplete
        }

        // Source
        const selectedSource = this.factories.find(
          (c) => c.id === this.breakdown_service.mechine_source_id
        );
        if (selectedSource) {
          this.breakdown_service.mechine_source_id = selectedSource.id; // Set the company_id for v-autocomplete
        }
        // Supplier
        const selectedSupplier = this.factories.find(
          (c) => c.id === this.breakdown_service.supplier_id
        );
        if (selectedSupplier) {
          this.breakdown_service.supplier_id = selectedSupplier.id; // Set the company_id for v-autocomplete
        }
        // Rent
        const selectedRent = this.factories.find(
          (c) => c.id === this.breakdown_service.rent_id
        );
        if (selectedRent) {
          this.breakdown_service.rent_id = selectedRent.id; // Set the company_id for v-autocomplete
        }
      } catch (error) {
    ...
    updatePreventiveServiceDays() {
      const selectedType = this.types.find(
        (type) => type.id === this.breakdown_service.mechine_type_id
      );
      console.log("Selected Type:", selectedType); // Debugging log

      this.breakdown_service.preventive_service_days = selectedType
        ? selectedType.day
        : "";
    },
