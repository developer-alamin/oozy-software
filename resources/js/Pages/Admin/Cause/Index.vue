...
export default {
  ...
  methods: {
    async loadItems({ page, itemsPerPage, sortBy }) {
      ...
      try {
        const response = await this.$axios.get("/cause", {
          params: {
            page,
            itemsPerPage,
            sortBy: sortKey,
    ...
    causeCreate() {
      this.$router.push({ name: "CauseCreate" });
    },
    viewTrash() {
      this.$router.push({ name: "CauseTrash" });
    },
    editCause(uuid) {
      this.$router.push({ name: "CauseEdit", params: { uuid } });
    },
    ...
    async confirmDelete() {
      this.dialog = false;
      try {
       const response = await this.$axios.delete(`/cause/${this.selectedCauseId}`);
        this.loadItems({
          page: 1,
    ...
    async fetchTrashedCausesCount() {
      try {
        const response = await this.$axios.get("/causes/trashed-count");
