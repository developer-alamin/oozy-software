<template>
  <v-card>
    <v-card-title class="pt-5">
      <v-row>
        <v-col cols="6"><span>Trashed Effects List</span></v-col>
    ...
    >
      ...
      <template v-slot:item.actions="{ item }">
        <v-icon @click="restoreEffect(item.uuid)" color="green" class="mr-2">mdi-restore</v-icon>
...
export default {
  ...
  data() {
    return {
      restoreDialogName: "Are you sure you want to restore this effect?",
  ...
  methods: {
    async loadItems({ page, itemsPerPage, sortBy }) {
      ...
      try {
        const response = await this.$axios.get("/effects/trashed", {
          params: {
            page,
            itemsPerPage,
            sortBy: sortKey,
    ...
    async confirmRestore() {
      ...
      try {
        await this.$axios.post(`/effects/${this.selectedEffectId}/restore`);
        this.loadItems({
          page: 1,
          itemsPerPage: this.itemsPerPage,
          sortBy: [],
        });
        toast.success("Effect restored successfully!");
      } catch (error) {
        console.error("Error restoring effect:", error);
        toast.error("Failed to restore effect.");
      }
    },
    async confirmDelete() {
      this.deleteDialog = false; // Close the delete dialog
      try {
        await this.$axios.delete(`/effects/${this.selectedEffectId}/force-delete`);
        this.loadItems({
          page: 1,
          itemsPerPage: this.itemsPerPage,
          sortBy: [],
