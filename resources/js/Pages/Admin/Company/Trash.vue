<template>
    <v-card>
        <v-card-title class="pt-5">
            <v-row>
                <v-col cols="6"><span>Company Trash List</span></v-col>
...
export default {
    ...
    data() {
        return {
            restroreDialogName: "Are you sure you want to restore this Company?",
            dialogName: "Are you sure you want to delete this Company ?",
            search: "",
            itemsPerPage: 15,
            headers: [
    ...
    methods: {
        async loadItems({ page, itemsPerPage, sortBy }) {
            ...
            try {
                const response = await this.$axios.get("/company/trash", {
                    params: {
                        page,
                        itemsPerPage,
                        sortBy: sortKey,
        ...
        async confirmRestore() {
            ...
            try {
                await this.$axios.post(
                    `/company/${this.selectedCompanyId}/restore`
                );
                this.loadItems({
                    page: 1,
                ...
                //bus.updateCount(bus.trashedCompanysCount.value - 1);
            } catch (error) {
        ...
        async confirmDelete() {
            ...
            try {
                await this.$axios.delete(
                    `/company/${this.selectedCompanyId}/forceDelete`
                );
                this.loadItems({
                    page: 1,
                    itemsPerPage: this.itemsPerPage,
